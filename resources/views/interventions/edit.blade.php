...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('web.interventions.update', $intervention->code_int) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="date" id="date_int" name="date_int"
                       value="{{ old('date_int', $intervention->date_int ? \Carbon\Carbon::parse($intervention->date_int)->format('Y-m-d') : '') }}"
                       required
            ...
            <div>
                ...
                <select id="code_user_client" name="code_user_client" required
                    ...
                    @foreach($clients as $client)
                        <option value="{{ $client->code_user }}"
                                {{ old('code_user_client', $intervention->code_user_client) == $client->code_user ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select id="code_user_techn" name="code_user_techn" required
                    ...
                    @foreach($techniciens as $technicien)
                        <option value="{{ $technicien->code_user }}"
                                {{ old('code_user_techn', $intervention->code_user_techn) == $technicien->code_user ? 'selected' : '' }}>
                ...
                <select id="code_comp" name="code_comp" required
                    ...
                    @foreach($competences as $competence)
                        <option value="{{ $competence->code_comp }}"
                                {{ old('code_comp', $intervention->code_comp) == $competence->code_comp ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="number" id="note_int" name="note_int"
                       value="{{ old('note_int', $intervention->note_int) }}"
            ...
            <div>
                ...
                <textarea id="commentaire_int" name="commentaire_int" rows="3"
                          class="w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500
                                 {{ $errors->has('commentaire_int') ? 'border-red-400 bg-red-50' : 'border-gray-300' }}">{{ old('commentaire_int', $intervention->commentaire_int) }}</textarea>
