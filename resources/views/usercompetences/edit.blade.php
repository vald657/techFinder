...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800">Modifier l'association</h1>
    </div>

    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('web.user-competences.update', $usercompetence) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
                ...
                <select id="code_user" name="code_user" required
                    ...
                    @foreach($utilisateurs as $utilisateur)
                        <option value="{{ $utilisateur->code_user }}"
                                {{ old('code_user', $usercompetence->code_user) == $utilisateur->code_user ? 'selected' : '' }}>
                ...
                <select id="code_comp" name="code_comp" required
                    ...
                    @foreach($competences as $competence)
                        <option value="{{ $competence->code_comp }}"
                                {{ old('code_comp', $usercompetence->code_comp) == $competence->code_comp ? 'selected' : '' }}>
            ...
            <div class="flex items-center space-x-3 pt-2">
                ...
                <a href="{{ route('web.user-competences.show', $usercompetence) }}"
